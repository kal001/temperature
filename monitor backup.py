...
def printHTMLHead(title, records, minimo, maximo, sens):
    ...
    <form method="POST" action="%s" id="changelanguage">
    <div style="float:right;">
      <select name="language" onchange="myFunction()">
        <option %s value="portugues">Português</option>
        <option %s value="english">English</option>
      </select>
    </div>
    </form>
...
def get_min(interval):
    ...
    conn = sqlite3.connect(dbname)
    curs = conn.cursor()
    query = "SELECT min(temp) FROM temps"
...
def get_max(interval):
    ...
    conn = sqlite3.connect(dbname)
    curs = conn.cursor()
    query = "SELECT max(temp) FROM temps"
...
def show_stats(option):
    ...
    # todo Acrescentar estatísticas apenas no intervalo em causa

    curs.execute("SELECT timestamp,max(temp),ID FROM temps")
    rowmax = curs.fetchone()
    rowstrmax = "{0}&nbsp&nbsp&nbsp{1:.1f} ºC&nbsp&nbsp&nbsp{2}".format(str(rowmax[0]), float(rowmax[1]),
                                                                        dictsensores[rowmax[2]])

    curs.execute("SELECT timestamp,min(temp),ID FROM temps")
    rowmin = curs.fetchone()
    rowstrmin = "{0}&nbsp&nbsp&nbsp{1:.1f} ºC&nbsp&nbsp&nbsp{2}".format(str(rowmin[0]), float(rowmin[1]),
                                                                        dictsensores[rowmin[2]])
    ...
    print _("<h2>Temperatura minima de sempre</h2>")
    print rowstrmin
    print _("<h2>Temperatura maxima de sempre</h2>")
    print rowstrmax
    print _("<h2>Temperatura media de sempre</h2>")
    print "%.1f" % rowavg + " ºC"

    print "<hr>"

    print _("<h2>Na ultima hora (todos os sensores)</h2>")
    print "<table>"
    print _(
...
def print_time_selector(option):
    print """
    <form name="escolhedatas" action="%s" method="POST">
      <script>
    ...
    print _("<strong>Seleccionar por datas</strong><br>")
...
def main():
    ...
    minimo = get_min(option)
    maximo = get_max(option)
    ...
    # print the page body
    print "<body>"
    print _("<h1>Temperatura ambiente em Casa vs %s</h1>") % APPVERSION
